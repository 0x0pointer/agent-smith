...
- **mobsf**: custom image — `docker build -t pentest-agent/mobsf ./tools/mobsf/` (thin wrapper over the official MobSF image). Auto-starts on first `scan(tool="mobsf")` call. REST API on port 5003. Used by `/android-security` & `/ios-security` for APK/IPA static analysis.
