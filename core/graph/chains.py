...
def candidate_chains(g: m.Graph) -> list[dict]:
    ...
    # (2) Composition: a credential/secret-leak finding + another finding on the
    #     SAME host → "leak creds, authenticate, then reach the second bug". This
    #     is the classic chain a model rarely self-assembles.
    ...
    for lk in leakers:
        ...
        for other in findings:
            if other.id == lk.id or _host(other.id) != h:
                continue
            props.append({
    ...
    # (3) A confirmed injection/RCE finding + a known credential/token on the host
